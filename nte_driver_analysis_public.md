...
This analysis is defensive throughout. The driver binary is **never executed on the static-analysis host** (a Linux box); §C1–§C5 verdicts and the cert-chain work in §C7 come from static analysis of the binary alone. §C8's runtime evidence is collected non-invasively from a disposable bare-metal Windows 11 host configured for this work — Secure Boot, VBS, and HVCI (Memory Integrity) enabled; no user data, no saved credentials, freshly imaged and treated as compromised post-test.

Specifically:

- The driver binary was obtained from the official NTE preload installer, run inside a sacrificial Windows 11 VMware VM on an isolated network segment, with no user data and no saved credentials. The binary was extracted inertly from that VM to a Linux analysis box.
- Static analysis on the Linux box: `pefile`, `lief`, `radare2`, `osslsigncode`, `capa`, and (in a follow-up pass) **Ghidra 12.0.4 in headless mode** — none of which executes the binary. The Linux box never opens a handle to the driver, never issues IOCTLs, never loads the driver. The Ghidra pass was added after a self-audit identified that several findings depended on string searches that could not detect stack-string-constructed API names; Ghidra's decompiler resolved that gap and is the basis for the final §C3 and §C5 verdicts.
- §C8 runtime verification on the disposable bare-metal Windows 11 host described above: the retail NTE installer was run, the launcher and game played to Main Menu and into gameplay, and the driver's load behavior observed via Code Integrity event log, SCM-based enumeration tools (`driverquery`, `Win32_SystemDriver`, `fltmc`), and Process Explorer (Sysinternals) running elevated. Observation is non-invasive — no kernel debugger, no driver hooking, no game instrumentation. The host is treated as compromised post-test.
- All static findings are reproducible by anyone with a copy of the same `.sys` file (SHA256 `9d89947b7ecaf8821df34fb6209b575b1a56a3257a0b225d7c63885fe1c4b569`); each Appendix A entry lists the exact read-only command that generates it. §C8 findings are reproducible by anyone with a similarly disposable Windows host running the retail NTE client.
